    ...
    <div className="pt-32 pb-20 space-y-24">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center space-y-6">
        <motion.div
          ...
          className="space-y-4"
        >
          <h1 className="text-5xl md:text-6xl font-bold text-slate-900">Pilgrim Resources</h1>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {RESOURCES.map((resource) => {
            ...
            return (
              <div key={resource.id} className="bg-white p-10 rounded-[2.5rem] border border-slate-200 shadow-sm hover:shadow-xl transition-all group">
                ...
                <p className="text-slate-500 leading-relaxed mb-8">{resource.description}</p>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div className="space-y-8">
              <h2 className="text-4xl font-bold text-white">Umrah Rituals Explained</h2>
              <div className="space-y-6">
                {[
                  ...
                  <div key={idx} className="flex gap-6 p-6 bg-white/5 rounded-2xl border border-white/10 hover:bg-white/10 transition-all">
                    <div className="w-10 h-10 rounded-full bg-gold-500 text-emerald-950 flex items-center justify-center font-bold shrink-0">
                      {idx + 1}
                    </div>
                    <div>
                      <h4 className="text-white font-bold mb-1">{ritual.title}</h4>
                      <p className="text-emerald-100/60 text-sm leading-relaxed">{ritual.desc}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="relative aspect-video rounded-3xl overflow-hidden shadow-2xl group cursor-pointer">
              ...
              <div className="absolute bottom-6 left-6 text-white font-bold">Watch: Umrah Rituals Guide</div>
            </div>
          </div>
        </div>
      </section>
